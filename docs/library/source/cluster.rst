...
Using |project| usually involves creating one or more ``Cluster``\s.
A cluster is created using a "backend", which might be Docker or a cloud provider for example.
It is also possible to point |project| to existing nodes.
...
:py:class:`~dcos_e2e.cluster.Cluster`\ s created with this method cannot be destroyed by |project|.
